...
\begin{itemize}
    ...
    \item $\dot{Q}_V$ - objemový zdroj tepla (\ueqWandMinvcu \fs -- watt na metr krychlový)
    ...
    \item $U_{\vartheta A}$ - prostup tepla (\ueqWandKinv \fs -- watt na kelvin)
    ...
    \item $G_e$ - elektrická vodivost (\ueqSIE \fs -- siemens)
    \item $\hat{Z}_e$ - impedance (\ueqOHM \fs -- ohm)
    \item $\hat{Y}_e$ - admitance (\ueqSIE \fs -- siemens)
    \item $B_e$ - elektrická susceptance (\ueqSIE \fs -- siemens)
    \item $L_e$ - elektrická indukčnost (\ueqHENRY \fs -- henry)
    \item $C_e$ - elektrická kapacita (\ueqFARAD \fs -- farad)
\end{itemize}
