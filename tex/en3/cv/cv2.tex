...
\begin{itemize}
    ...
    \item měrný součinitel prostupu tepla střechy $u_{strecha} = 0,3 \fs \uWandMinvsqKinv$,
    \item plocha podlahy $S_{podlaha} = 100 \fs \uMsq$,
    \item měrný součinitel prostupu tepla podlahy $u_{podlaha} = 0,8 \fs \uWandMinvsqKinv$,
...
\begin{enumerate}[a)]
    \item Vypočítejte průměrný měrný součinitel prostupu tepla $u_{avg}$ pro celý dům.
    \item Vypočítejte jak silnou izolaci $d_{izol}$ s tepelnou vodivostí $\lambda_{izol} = 0,04 \fs \uWandMinvKinv$ musíte použít, aby průměrný měrný součinitel prostupu tepla $u_{avg}^{*}$ byl $0,5 \fs \uWandMinvsqKinv$.
\end{enumerate}
...
Měrný součinitel prostupu tepla zdi $u_{cihla}$ vypočteme jako:
$$
    u_{cihla} = \frac{\lambda_{cihla}}{d_{cihla}} = \frac{0,8 \fs \uWandMinvKinv}{0,45 \fs \uM} = 1,778 \fs \uWandMinvsqKinv.
$$

Průměrný měrný součinitel prostupu tepla $u_{avg}$ vypočteme jako:
$$
    u_{avg} = \frac{1}{S_{\Sigma}} \cdot (u_{strecha} \cdot S_{strecha} + u_{podlaha} \cdot S_{podlaha} + u_{cihla} \cdot S_{zed}) =
$$
$$
...
\subsubsection{b}
Měrný součinitel prostupu tepla zdi s izolací $u_{zed}$ vypočteme jako:
$$
    u_{zed} = \left( \frac{d_{izol}}{\lambda_{izol}} + \frac{d_{cihla}}{\lambda_{cihla}} \right)^{-1}
$$

Průměrný měrný součinitel prostupu tepla $u_{avg}^{*}$ vypočteme jako:
$$
    u_{avg}^{*} = \frac{1}{S_{\Sigma}} \cdot (u_{strecha} \cdot S_{strecha} + u_{podlaha} \cdot S_{podlaha} + u_{zed} \cdot S_{zed}) =
$$
$$
    = \frac{1}{S_{\Sigma}} \cdot (u_{strecha} \cdot S_{strecha} + u_{podlaha} \cdot S_{podlaha} + \left( \frac{d_{izol}}{\lambda_{izol}} + \frac{d_{cihla}}{\lambda_{cihla}} \right)^{-1} \cdot S_{zed})
$$

Nyní je třeba vyřešit rovnici pro $d_{izol}$:
$$
    u_{avg}^{*} = \frac{1}{S_{\Sigma}} \cdot (u_{strecha} \cdot S_{strecha} + u_{podlaha} \cdot S_{podlaha} + \left( \frac{d_{izol}}{\lambda_{izol}} + \frac{d_{cihla}}{\lambda_{cihla}} \right)^{-1} \cdot S_{zed})
$$
$$
    u_{avg}^{*} \cdot S_{\Sigma} =  (u_{strecha} \cdot S_{strecha} + u_{podlaha} \cdot S_{podlaha} + \left( \frac{d_{izol}}{\lambda_{izol}} + \frac{d_{cihla}}{\lambda_{cihla}} \right)^{-1} \cdot S_{zed})
$$
$$
    u_{avg}^{*} \cdot S_{\Sigma} - u_{strecha} \cdot S_{strecha} - u_{podlaha} \cdot S_{podlaha} = \left( \frac{d_{izol}}{\lambda_{izol}} + \frac{d_{cihla}}{\lambda_{cihla}} \right)^{-1} \cdot S_{zed}
$$
$$
    \left( \frac{d_{izol}}{\lambda_{izol}} + \frac{d_{cihla}}{\lambda_{cihla}} \right)^{-1} = \frac{u_{avg}^{*} \cdot S_{\Sigma} - u_{strecha} \cdot S_{strecha} - u_{podlaha} \cdot S_{podlaha}}{S_{zed}}
$$
$$
    \frac{d_{izol}}{\lambda_{izol}} + \frac{d_{cihla}}{\lambda_{cihla}} = \left( \frac{u_{avg}^{*} \cdot S_{\Sigma} - u_{strecha} \cdot S_{strecha} - u_{podlaha} \cdot S_{podlaha}}{S_{zed}} \right)^{-1}
$$
$$
    \frac{d_{izol}}{\lambda_{izol}} = \left( \frac{u_{avg}^{*} \cdot S_{\Sigma} - u_{strecha} \cdot S_{strecha} - u_{podlaha} \cdot S_{podlaha}}{S_{zed}} \right)^{-1} - \frac{d_{cihla}}{\lambda_{cihla}}
$$
$$
    d_{izol} = \lambda_{izol} \cdot \left( \left( \frac{u_{avg}^{*} \cdot S_{\Sigma} - u_{strecha} \cdot S_{strecha} - u_{podlaha} \cdot S_{podlaha}}{S_{zed}} \right)^{-1} - \frac{d_{cihla}}{\lambda_{cihla}} \right)
$$
